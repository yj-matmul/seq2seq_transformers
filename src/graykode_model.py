...
def get_sinusoid_encoding_table(n_position, d_model):
    ...
    return torch.FloatTensor(sinusoid_table).to(cur_device)
...
def get_attn_subsequent_mask(seq):
    ...
    subsequent_mask = torch.from_numpy(subsequent_mask).byte().to(cur_device)
...
class MultiHeadAttention(nn.Module):
    def __init__(self):
        ...
        self.W_Q = nn.Linear(d_model, d_k * n_heads).to(cur_device)
...
class PoswiseFeedForwardNet(nn.Module):
    def __init__(self):
        ...
        # self.conv1 = nn.Conv1d(in_channels=d_model, out_channels=d_ff, kernel_size=1).to(cur_device)
...
class Encoder(nn.Module):
    ...
    def forward(self, enc_inputs): # enc_inputs : [batch_size x source_len]
        enc_outputs = self.src_emb(enc_inputs) + self.pos_emb(torch.LongTensor([[1,2,3,4,0]]).to(cur_device))
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.tgt_emb = nn.Embedding(tgt_vocab_size, d_model).to(cur_device)
        self.pos_emb = nn.Embedding.from_pretrained(get_sinusoid_encoding_table(tgt_len+1, d_model),freeze=True).to(cur_device)
